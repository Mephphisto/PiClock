...
async def clock_dark_color(ha: home_assistant.HaClient) -> Image.Image:
    ...
    draw.text(
        (240, 240), time_s, (255, 255, 255, 255), font=font, anchor="ms", outline=None
    )
...
async def clock_triangles(ha: home_assistant.HaClient) -> Image.Image:
    ...
    draw.regular_polygon(
        (center, radius), 3, rotation=-t / 7 % 360, fill=(255), outline=None, width=1
    )
    draw = ImageDraw.Draw(green)
    draw.regular_polygon(
        (center, radius), 5, rotation=t / 11 % 360, fill=(255), outline=None, width=1
    )
    draw = ImageDraw.Draw(red)
    draw.regular_polygon(
        (center, radius), 4, rotation=t / 5 % 360, fill=(255), outline=None, width=1
    )
    channels = [
...
async def _draw_ha(drw: ImageDraw.Draw, ha: home_assistant.HaClient, color):
    ...
    try:
        ...
        drw.text(
            (RESOLUTION[0] - 260, RESOLUTION[1] - 80),
            baro,
            color,
            font=font,
            anchor="lm",
        ...
        drw.text(
            (RESOLUTION[0] - 260, RESOLUTION[1] - 30),
            percip,
            color,
            font=font,
            anchor="lm",
